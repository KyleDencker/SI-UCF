...
public class GameController extends ApplicationAdapter {
	...
	Music music;
	Sound enemyGoesBoom;
	ArrayList<ParticleEffect> fires;
	TextureAtlas atlas;

	BitmapFont windgings, other;
	int count;
	...
	public void create () {
		...
		fires = new ArrayList<ParticleEffect>();

		// Creates the fonts
		windgings = new BitmapFont(Gdx.files.internal("fonts/csans.fnt"), false);
		windgings.setColor(new Color(0.3f, 0.9f, 0.5f, 1.0f));
		other = new BitmapFont(Gdx.files.internal("fonts/other.fnt"), false);
		// counts the number of enemies destroyed
		count = 0;
	}

	// creates a new partiocle
	public void newFire(float x, float y) {
		ParticleEffect p = new ParticleEffect();
		p.load(Gdx.files.internal("MURICA.p"), Gdx.files.internal("particles"));
		p.allowCompletion();
		p.setDuration(100);
		p.setPosition(x, y);
		p.start();
	...
	public void render () {
		...
		player.draw(batch);
		for (ParticleEffect p : fires) p.draw(batch, Gdx.graphics.getDeltaTime());
		// draw the two fonts
		windgings.draw(batch, "Num destroyed: " + count, 50, 50);
		other.draw(batch, "num in lazer list: " + lazerList.size(), 500, 500);
		...
		enemyLoop : for (int i=0; i<enemies.size(); i++) {
			...
			for (int j=0; j<lazerList.size(); j++) {
				if (enemies.get(i).hitBox.overlaps(lazerList.get(j).hitBox)) {
					// creates a particle when an enemy is hit
					newFire(enemies.get(i).hitBox.x, enemies.get(i).hitBox.y);
					enemies.remove(i);
					i--;
					lazerList.remove(j);
					j--;
