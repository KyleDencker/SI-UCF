...
public class MainScreen implements Screen {
	...
	public Texture whiteSquare, blackSquare, blueSquare, greenSquare, graySquare, yellowSquare;
	public char[][] grid;
	public int[][] dist;
	public Random rand;
	public char type;
	...
	public void render(float delta) {
		...
		for (int y = 0; y < 12; y++) {
			for (int x = 0; x < 20; x++) {
				...
				if (grid[y][x] == 'P') batch.draw(yellowSquare, 40*x, 40*y);
			}
		}
		batch.end();
	...
	public void reset() {
		for (int y = 0; y < GameHandler.ROWS; y++) {
			for (int x = 0; x < GameHandler.COLUMNS; x++) {
				if (grid[y][x] == 'V' || grid[y][x] == 'P') grid[y][x] = 'F';
				dist[y][x] = -1;
			}
		}
	}
	...
	public void printDist() {
		for (int y = GameHandler.ROWS - 1; y >= 0; y--) {
			for (int x = 0; x < GameHandler.COLUMNS; x++) {
				String s = String.valueOf(dist[y][x]);
