...
public class GameController extends ApplicationAdapter {
	...
	public void render () {
			...
			switch (hud.checkInput(hudPos.x, hudPos.y)) {
				...
				case -1:
					if (Gdx.input.justTouched()) {
						...
						Queue<Vector2> path = visibilityGraph.findPath(tank.getPosition().x, tank.getPosition().y, targetPos.x, targetPos.y);
						if (path.size > 0)
							tank.setPath(path);
						break;
					}
		...
		for (Gem g : gems) {
			if (tank.getHitBox().overlaps(g.getHitBox())) {
				gemPool.free(g);
				gems.removeValue(g, true);
				spawnGem();
				tank.addPoints(g.getValue());
			}
		}
	...
	private void addMines () {
		for (int i = 0; i < 50; i++) {
			...
			do {
				...
				validPlacement = !(new Vector2(x, y).dst(spawnPoint.getRectangle().getX(), spawnPoint.getRectangle().getY()) < 100);
