...
import BoardObjects.DialogueBox;
...
public class MainGame implements Screen {
    ...
    public void show() {
        ...
        chatTimer = 0;
        box = new DialogueBox(block_w*9, block_h);
    ...
    public void render(float delta) {
        ...
        /*
        if(isChatting){
            chatTimer += Gdx.graphics.getDeltaTime();
            if(chatTimer > 3){
                chatTimer = 0;
                isChatting = false;
            }
        ...
        */
        ...
        if(isChatting){
            box.draw(batch);
            for(int l = 0; l<2; l++){
                bitmapFont.draw(batch, box.options[l],box.positions[l].x, box.positions[l].y);
            }
            //bitmapFont.draw(batch, chat, pknight.x + block_w/2, pknight.y + block_h);
        }
        ...
        if(isChangingRoom){
            ...
            if(Gdx.input.isKeyJustPressed(Input.Keys.Q)){
                ...
                for(Enemy enemy : crntRoom.enemies){
                    if(pknight.chatRect.overlaps(enemy.rect)){
                        isChatting = true;
                        chat = "hi";
                    }
                }
